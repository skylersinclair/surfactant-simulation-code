...
# Number of spacial dimensions (2 for x and y directions)
    ndim = 2
...
# when performing iteration, don't exceed 30 iterations per timestep
    probdata.add_param('newton_max_iter', 30)
# Use newton's method to converge to the value that satisfies
# the Crank-Nicolson method each iteration
# Newton used for nonlinear components
    probdata.add_param('newton_tolerance', 1e-4)
    probdata.add_param('newton_verbosity', 2)
# How close two iterations must be for the computer to decide it's converged
# for linear piece
    probdata.add_param('linear_solver_tolerance', 1e-4)
    probdata.add_param('linear_solver_verbosity', 1)
# Threads used for a multicore machine if want to run on multiple threads at once
    probdata.add_param('num_threads', 4)
# p is the periodic boundary conditions. Must be coordinated between upper and 
# lower ends of the domain
# This means that when the flow hits one boundary, it gets booted into 
# the opposite boundary (just means that we're basically ignoring the edges).

# 'n', or none (not used here, but can be) creates and lls ghost cells on outside 
# edges that are set equal to the boundary cells making the first and third derivatives
# of those boundary cells 0 (at least that's a possible implementation?)
